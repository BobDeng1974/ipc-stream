...
static int dataCallback(void *opaque, void *pData, int nDataLen, int nFlag, int64_t timestamp, int nIsKeyFrame)
{
        ...
        if (nFlag == THIS_IS_AUDIO){
                ...
                ret = pStream->audioCb( (char *)pData, nDataLen, (double)timestamp, 0, NULL );
        } else {
                ...
                ret = pStream->videoCb(pStream->nStreamNo, (char *)pData, nDataLen, nIsKeyFrame, (double)timestamp, 0, 0, 0);
        }
        return ret;
}
...
static int SimDevInitIPC( int audioType, int subStreamEnable, VideoFrameCb videoCb, AudioFrameCb audioCb )
{
    ...
    s32Ret = simdev_init();
    if ( s32Ret < 0 ) {
        LOGE("simdev_init error, s32Ret = %d\n", s32Ret );
        return -1;
    }
...
int SimDevStartStream()
{
    ...
    LOGI("gSimDevCaptureDev.mainContext = %d\n", gSimDevCaptureDev.mainContext );
